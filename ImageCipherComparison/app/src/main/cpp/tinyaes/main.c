...
int main(int argc, char* argv[]) {
	...
	if(numberOfBytes <= AES_BLOCK_SIZE)
		...
		if(numberOfBytes % AES_BLOCK_SIZE == 0)
			paddedSize = numberOfBytes;
		else
		    paddedSize = numberOfBytes / AES_BLOCK_SIZE + AES_BLOCK_SIZE;
	}

	BYTE *padded_buffer_in = (BYTE*)malloc(sizeof(BYTE)*numberOfBytes);
	BYTE *padded_buffer_out = (BYTE*)malloc(sizeof(BYTE)*numberOfBytes); 

	printf("input number of bytes (%d): \n", numberOfBytes);

	for(int i = 0; i < paddedSize; i++){
		if(i < numberOfBytes)
			padded_buffer_in[i] = origImageBytes[i];
		else
	...
	for(int i = 0; i < paddedSize; i++) {
		printf("%u ", padded_buffer_out[i]);
		padded_buffer_in[i] = 0;
	}
	printf("\n------------------\n");
	...
	proccessedBytes = 0;
	while(proccessedBytes < paddedSize) {
		aes_decrypt(&padded_buffer_out[currentBlock*AES_BLOCK_SIZE], &padded_buffer_in[currentBlock*AES_BLOCK_SIZE], key_schedule, AES_KEY_SIZE);
		proccessedBytes += AES_BLOCK_SIZE;
		currentBlock ++;
	}
	...
	//aes_encrypt(padded_buffer_in, paddedSize, padded_buffer_out, key_schedule, 256);
